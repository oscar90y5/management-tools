...
class TestBurnDown(TestCase):
    def setUp(self) -> None:
        ...
        self._init_current_sprint_with_issues_on_progress()
    ...
    def _init_current_sprint(self):
        issues_dict = {
            ...
            'Estado': ['Cerrada', 'Cerrada', 'Cerrada', 'Nueva', 'Nueva', 'Nueva', 'Nueva']
        }
        issues_df = pandas.DataFrame.from_dict(issues_dict)
        issues_df['Cerrada'] = pandas.to_datetime(issues_df['Cerrada'])
    ...
    def _init_current_sprint_with_issues_on_progress(self):
        issues_dict = {
            'Story points': [1] * 7,
            'Prioridad': ['Alta'] * 7,
            'Cerrada': [
                '2021-5-03 12:00', '2021-5-04 12:00',
            ] + 5 * [None],
            'Estado': ['Cerrada', 'Cerrada', 'En curso', 'En curso', 'Resuelta', 'Comentarios', 'Nueva']
        }
        issues_df = pandas.DataFrame.from_dict(issues_dict)
        issues_df['Cerrada'] = pandas.to_datetime(issues_df['Cerrada'])

        self.current_sprint_with_issues_on_progress = SprintFactory(
            start_date='3/5/21',
            pre_deployment_date='12/5/21',
            end_date='14/5/21',
            issues_df=issues_df
    ...
    @freeze_time("2021-05-05")
    def test_current_sprint_with_issues_on_progress(self):
        fig, ax = plt.subplots()

        self.current_sprint_with_issues_on_progress.plot_burndown(fig, ax)
